...
import { communityApi, ideasApi, challengesApi } from '@/api/client';
...
function LeaderboardTab() {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 0 40px' }}>
      ...
      {/* Top 3 podium cards */}
      {top3.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : `repeat(${top3.length}, 1fr)`, gap: 12, marginBottom: 24 }}>
          {top3.map((e, i) => {
            const isFirst = i === 0;
            return (
              <div
    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: '0 16px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 210px 1fr', gap: '0 0', alignItems: 'stretch' }}>
  ...
  const handleReact = async (id: string, type: 'encourage' | 'pursue') => {
    ...
    setPainPoints(prev => prev.map(p => {
      ...
      return {
        ...
        pursue_count:    type === 'pursue'    ? (p.pursue_count ?? 0) + (wasThis ? -1 : 1) : p.pursue_count,
      };
    }));
  };
...
export default function CommunityPage() {
  ...
  return (
    ...
    <div style={{ maxWidth: 1360, margin: '0 auto', padding: isMobile ? '20px 16px 60px' : '32px 40px 80px', color: LIT.text }}>
